-- ===========================================================================
-- LASSI — Fitness classement : compter les abonnements ACTIFS pendant la période
-- ---------------------------------------------------------------------------
-- Problème : cmds_fitness filtrait par date_achat >= week_start
-- → Un abonné acheté la semaine précédente disparaît du classement la semaine suivante
-- → Les fitness prestataires voient toujours 0 pts sauf la semaine d'achat
--
-- Fix : pour SPORT/FITNESS, on compte les abonnements dont la plage de validité
-- CHEVAUCHE la période (date_achat <= fin_période AND date_expiration >= début_période)
-- Cela reflète "combien de clients actifs ce prestataire sert cette semaine/mois"
-- et augmente naturellement dès qu'un nouvel abonnement est souscrit.
--
-- Cohérent avec le dashboard (qui montre les recettes du mois) :
-- les abonnements se remettent à 0 quand ils expirent tous.
-- ===========================================================================


-- ── 1. get_classement_live_sous_categorie (Ma catégorie – Cette semaine) ────
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  -- Abonnements actifs PENDANT cette semaine (chevauche la fenêtre)
  -- Compte les abonnés en cours même achetés les semaines précédentes
  cmds_fitness AS (
    SELECT s.id AS shop_id
    FROM fitness_abonnements_clients fac
    JOIN shops s ON s.merchant_id = fac.prestataire_id
    CROSS JOIN week_bounds wb
    WHERE fac.statut = 'actif'
      AND fac.date_achat      <= wb.week_end    -- acheté avant la fin de cette semaine
      AND fac.date_expiration >= wb.week_start  -- encore valide cette semaine
  ...
  cmds_all AS (
    ...
    UNION SELECT shop_id FROM cmds_fitness
...
-- ── 2. get_classement_live_mondial (National – Ce mois-ci) ──────────────────
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  -- Abonnements actifs PENDANT ce mois
  cmds_fitness AS (
    ...
    WHERE fac.statut = 'actif'
      AND fac.date_achat      <= mb.mois_end    -- acheté avant la fin de ce mois
      AND fac.date_expiration >= mb.mois_start  -- encore valide ce mois
      AND s.merchant_id IS NOT NULL
  ),
  cmds_terrain AS (
  ...
  cmds_all AS (
    ...
    UNION SELECT shop_id, client_id FROM cmds_fitness
...
-- ── 3. calcul_classements_semaine (cron dimanche) ────────────────────────────
...
BEGIN
  ...
  cmds_fitness AS (
    ...
    WHERE fac.statut = 'actif'
      ...
      AND fac.date_expiration >= v_week_start
      AND s.merchant_id IS NOT NULL
  ),
  cmds_terrain AS (
    SELECT s.id AS shop_id, rt.client_id
...
-- ── 4. calcul_classements_mois (cron mensuel) ────────────────────────────────
...
BEGIN
  ...
  cmds_fitness AS (
    ...
    JOIN shops s ON s.merchant_id = fac.prestataire_id
    WHERE fac.statut = 'actif'
      AND fac.date_achat      <= v_mois_end
      AND fac.date_expiration >= v_mois_start
      AND s.merchant_id IS NOT NULL
  ),
  cmds_terrain AS (
    SELECT s.id AS shop_id, rt.client_id
  ...
  client_fitness_c AS (
    ...
    FROM fitness_abonnements_clients fac
    WHERE fac.statut='actif'
      AND fac.date_achat<=v_mois_end AND fac.date_expiration>=v_mois_start
    GROUP BY fac.client_id
  ),
  client_terrain_c AS (
    SELECT rt.client_id, COUNT(*) AS nb_terrain FROM reservations_terrain rt
    WHERE rt.statut='paye' AND rt.created_at>=v_mois_start AND rt.created_at<v_mois_end AND rt.client_id IS NOT NULL
...
BEGIN
  ...
  RAISE NOTICE '[lassi] semaine recalculé (abos actifs) : % lignes', v_n;
  SELECT public.calcul_classements_mois() INTO v_n;
  RAISE NOTICE '[lassi] mois recalculé (abos actifs) : % lignes', v_n;
