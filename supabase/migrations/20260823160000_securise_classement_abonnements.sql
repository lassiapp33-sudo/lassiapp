-- ===========================================================================
-- LASSI — Classement abonnements : sécurisation + généralisation
-- ---------------------------------------------------------------------------
-- 1. Anti-fraude : cap 5 achats par client par boutique par semaine/mois
--    (un seul client ne peut pas faire monter artificiellement un prestataire)
-- 2. Paiement réel : payment_intent_id IS NOT NULL dans les abonnements
--    (uniquement les abonnements issus d'un vrai paiement OM/Wave)
-- 3. Généralisation : la table fitness_abonnements_clients s'applique à
--    TOUT prestataire proposant des abonnements, quelle que soit sa catégorie
--    (sport, bien-être, beauté, coiffure, etc.)
-- ===========================================================================
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  -- Commandes régulières payées cette semaine
  cmds_orders AS (
  ...
  -- Abonnements achetés cette semaine — TOUT type de prestataire
  -- (sport, bien-être, beauté…) dès qu'il y a un paiement réel
  cmds_abonnements AS (
  ...
  -- Réservations terrain payées cette semaine
  cmds_terrain AS (
  ...
  -- Réservations table VIP payées cette semaine
  cmds_table_resa AS (
  ...
  -- Sécurité anti-fraude : un même client compte au max 5 fois par boutique
  cmds_ranked AS (
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  cmds_abonnements AS (
    ...
    WHERE fac.date_achat >= mb.mois_start
      AND fac.date_achat <  mb.mois_end
      AND fac.statut = 'actif'
      AND fac.payment_intent_id IS NOT NULL
      AND s.merchant_id IS NOT NULL
  ),
...
-- ── 3. calcul_classements_semaine (snapshot pg_cron – hebdomadaire) ───────────
...
-- ── 4. calcul_classements_mois (snapshot pg_cron – mensuel) ──────────────────
...
BEGIN
  ...
  cmds_terrain AS (
    ...
    WHERE rt.statut = 'paye' AND rt.created_at >= v_mois_start AND rt.created_at < v_mois_end
  ...
  scores AS (
    SELECT s.merchant_id AS prestataire_id,
      s.name AS nom, s.logo_url AS img, s.rating, s.created_at AS shop_created,
      ROUND(
        COALESCE(cv.nb_cmds,0)*10 + s.reviews_count*3 + s.rating*SQRT(GREATEST(s.reviews_count,0)+1)*2
      ) AS points
    FROM shops s LEFT JOIN cmds_valides cv ON cv.shop_id = s.id
    WHERE COALESCE(s.vip_exclu, FALSE) = FALSE AND s.merchant_id IS NOT NULL
  ),
...
BEGIN
  ...
  RAISE NOTICE '[lassi] semaine (sécurisé + générique) : % lignes', v_n;
  SELECT public.calcul_classements_mois() INTO v_n;
  RAISE NOTICE '[lassi] mois (sécurisé + générique) : % lignes', v_n;
