-- ===========================================================================
-- LASSI — Classement : compter les commandes Wave/OM payées (pas seulement 'done')
-- ---------------------------------------------------------------------------
-- Bug : cmds_orders filtrait status = 'done'.
--   Après paiement Wave, les commandes passent en 'new' (visible préstataire)
--   puis 'preparing' → 'ready' → 'done' au fur et à mesure du traitement.
--   Résultat : 0 pts dans le classement même avec 15 commandes payées le matin.
--
-- Fix : status IN ('new', 'preparing', 'ready', 'done') AND pay_method IN ('wave', 'om')
--   = toute commande dont le paiement est confirmé, quelle que soit l'avancement.
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  cmds_orders AS (
    ...
    WHERE o.status IN ('new', 'preparing', 'ready', 'done')
...
-- ── 3. Recalcul immédiat semaine courante ─────────────────────────────────────

DO $recalc$
DECLARE v_n INTEGER;
BEGIN
  SELECT public.calcul_classements_semaine() INTO v_n;
  RAISE NOTICE '[lassi] classement semaine recalculé : % lignes', v_n;
