-- ===========================================================================
-- FIX CRITIQUE : initiate_order_payment — accepter status 'pending'
-- ---------------------------------------------------------------------------
-- Problème :
--   Migration 20260808020000 a modifié create_order_atomic pour créer les
--   commandes Wave/OM avec status='pending' (invisible au prestataire avant
--   confirmation de paiement).
--   Mais initiate_order_payment vérifiait UNIQUEMENT status='new', donc
--   toute commande Wave/OM retournait 'order_not_payable' immédiatement →
--   erreur "Cette commande n'est plus en attente de paiement" côté client.
--
-- Fix :
--   Accepter status IN ('new', 'pending') — 'new' pour les commandes cash
--   converties, 'pending' pour les commandes Wave/OM en attente de paiement.
--
-- Invariant conservé :
--   - 'confirmed'/'split_done'/'simulated'/'disputed' → already_paid (bloqué)
--   - 'preparing'/'ready'/'done'/'refused' → order_not_payable (bloqué)
--   - Seuls 'new' et 'pending' = commande payable
-- ===========================================================================
...
BEGIN
  ...
  -- ★ FIX : accepter 'pending' (commandes Wave/OM créées par create_order_atomic
  --   depuis 20260808020000) ET 'new' (commandes cash ou anciennes commandes).
  --   Tout autre statut = commande non payable.
  ...
  -- Bloquer si paiement déjà abouti
