-- ===========================================================================
-- LASSI — Classement : exclure admin + cascade suppression boutique
-- ---------------------------------------------------------------------------
-- 1. Toutes les fonctions classement filtrent is_admin_account = FALSE
--    (colonne déjà gérée par trigger sync_shop_admin_account)
-- 2. Trigger AFTER DELETE sur profiles → supprime les boutiques orphelines
-- 3. Nettoyage immédiat des boutiques dont le prestataire n'existe plus
-- ===========================================================================


-- ── 1. Trigger cascade : suppression profile → suppression shops ──────────────

CREATE OR REPLACE FUNCTION public.on_profile_deleted_cleanup_shops()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  DELETE FROM public.shops WHERE merchant_id = OLD.id;
  RETURN OLD;
END;
$$;

DROP TRIGGER IF EXISTS trg_profile_deleted_cleanup_shops ON public.profiles;
CREATE TRIGGER trg_profile_deleted_cleanup_shops
  AFTER DELETE ON public.profiles
  FOR EACH ROW
  EXECUTE FUNCTION public.on_profile_deleted_cleanup_shops();


-- ── 2. Nettoyage immédiat des boutiques orphelines ────────────────────────────
-- (merchant_id pointe vers un profil supprimé)

DELETE FROM public.shops
WHERE merchant_id IS NOT NULL
  AND merchant_id NOT IN (SELECT id FROM public.profiles);


-- ── 3. get_classement_live_sous_categorie — filtrer admin ────────────────────
...
-- ── 4. get_classement_live_mondial — filtrer admin ───────────────────────────
...
-- ── 5. calcul_classements_semaine — filtrer admin ────────────────────────────
...
BEGIN
  ...
  cmds_terrain AS (
    ...
    WHERE rt.statut = 'paye' AND rt.created_at >= v_week_start AND rt.created_at < v_week_end
      AND rt.client_id IS NOT NULL AND s.merchant_id IS NOT NULL
  ),
  cmds_table_resa AS (
    SELECT vp.shop_id, tr.client_id
...
-- ── 6. calcul_classements_mois — filtrer admin ───────────────────────────────
...
BEGIN
  ...
  RAISE NOTICE '[lassi] semaine (sans admin, sans orphelins) : % lignes', v_n;
  SELECT public.calcul_classements_mois() INTO v_n;
  RAISE NOTICE '[lassi] mois (sans admin, sans orphelins) : % lignes', v_n;
