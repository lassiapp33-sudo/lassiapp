-- ===========================================================================
-- LASSI — Fitness classement : règle finale
-- ---------------------------------------------------------------------------
-- Règle métier validée :
--   • Achat d'un abonnement cette semaine → prestataire reçoit des pts immédiatement
--   • Ces pts sont valables UNIQUEMENT pour la semaine de l'achat (date_achat)
--   • Lundi suivant → repart à 0 (nouvelle semaine, nouveaux achats)
--   • Pas de lien avec la durée de l'abonnement
--   • Pas besoin de payout 'paid' : les pts arrivent dès l'activation
--
-- National (mensuel) : même logique mais fenêtre = mois courant
-- ===========================================================================
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  -- Abonnements ACHETÉS cette semaine → pts immédiats dès l'achat
  cmds_fitness AS (
...
BEGIN
  ...
  client_fitness_c AS (
    ...
    FROM fitness_abonnements_clients fac
    WHERE fac.date_achat>=v_mois_start AND fac.date_achat<v_mois_end AND fac.statut='actif'
...
BEGIN
  ...
  RAISE NOTICE '[lassi] semaine (date_achat semaine) : % lignes', v_n;
  SELECT public.calcul_classements_mois() INTO v_n;
  RAISE NOTICE '[lassi] mois (date_achat mois) : % lignes', v_n;
