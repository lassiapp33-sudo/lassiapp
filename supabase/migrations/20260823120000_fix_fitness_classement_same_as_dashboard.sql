-- ===========================================================================
-- LASSI — Fitness classement : même logique que get_daily_fitness_earnings
-- ---------------------------------------------------------------------------
-- Problème : cmds_fitness_by_payout joignait fitness_abonnements_clients→payout_queue
-- via fac.payment_intent_id. Si payment_intent_id est NULL sur certaines lignes
-- de fitness_abonnements_clients, la jointure retourne 0 lignes silencieusement.
--
-- Fix : on utilise EXACTEMENT le même chemin que get_daily_fitness_earnings :
--   payout_queue → payment_intents (via pq.payment_intent_id)
--   et on identifie les payouts fitness par pi.metadata->>'offre_nom' IS NOT NULL
-- De cette façon : si le dashboard montre 27F, le classement voit la même chose.
-- ===========================================================================
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  -- Abonnements achetés cette semaine (chemin direct, sans payout)
  ...
  -- Même logique que get_daily_fitness_earnings : payout_queue → payment_intents
  -- Couvre les payouts fitness PAYÉS cette semaine, indépendamment de date_achat
  cmds_fitness_by_payout AS (
    SELECT s.id AS shop_id
    FROM payout_queue pq
    JOIN payment_intents pi ON pi.id = pq.payment_intent_id
    JOIN shops s ON s.merchant_id = pq.prestataire_id
    CROSS JOIN week_bounds wb
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  -- Abonnements achetés ce mois (chemin direct)
  cmds_fitness_by_date AS (
  ...
  -- Même logique que get_daily_fitness_earnings : payout_queue → payment_intents
  cmds_fitness_by_payout AS (
    SELECT s.id AS shop_id, pi.client_id
    FROM payout_queue pq
    JOIN payment_intents pi ON pi.id = pq.payment_intent_id
    JOIN shops s ON s.merchant_id = pq.prestataire_id
    CROSS JOIN month_bounds mb
...
BEGIN
  ...
  client_fitness_by_payout AS (
    SELECT pi.client_id, COUNT(*) AS nb_abo
    FROM payout_queue pq JOIN payment_intents pi ON pi.id = pq.payment_intent_id
    WHERE pq.processed_at >= v_mois_start AND pq.processed_at < v_mois_end
      AND pq.statut = 'paid' AND (pi.metadata->>'offre_nom') IS NOT NULL
...
BEGIN
  ...
  RAISE NOTICE '[lassi] semaine recalculé (dashboard-aligned) : % lignes', v_n;
  SELECT public.calcul_classements_mois() INTO v_n;
  RAISE NOTICE '[lassi] mois recalculé (dashboard-aligned) : % lignes', v_n;
