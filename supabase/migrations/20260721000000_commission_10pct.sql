-- ===========================================================================
-- Commission LASSİ : passage de 1% à 10%
-- 1. Contrainte CHECK payment_intents mise à jour (NOT VALID = ne revalide
--    pas les anciennes lignes créées à 1%)
-- 2. create_payment_intent    → 10%
-- 3. initiate_order_payment   → 10% (redéfinition de la version 20260716220000)
-- 4. creer_commande_alaune    → 10%
-- ===========================================================================
...
ALTER TABLE public.payment_intents
  ADD CONSTRAINT check_montants CHECK (
    commission_lassi = CEIL(prix_base * 0.10) AND
...
BEGIN
  ...
  -- 10% LASSI
  v_commission := CEIL(p_prix_base * 0.10);
...
-- ─── 3. initiate_order_payment (dernière version = 20260716220000) ─────────
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'commission',        CEIL(v_prix_base * 0.10)::int,
    'montant_total',     v_prix_base + CEIL(v_prix_base * 0.10)::int,
...
BEGIN
  ...
  v_commission   := CEIL(v_total * 0.10);
