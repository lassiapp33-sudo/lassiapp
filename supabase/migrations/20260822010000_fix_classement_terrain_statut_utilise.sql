-- =============================================================================
-- FIX : classements terrain — statut 'utilise' ignoré dans tous les CTEs
-- -----------------------------------------------------------------------------
-- Cause : verify_terrain_receipt() met statut='utilise' après scan QR.
--         Tous les CTEs cmds_terrain filtraient uniquement statut='paye' →
--         réservations validées disparaissent → nb_cmds=0 → score figé à
--         reviews_count*3 (ex: 30 pts si reviews_count=10) pour tout le monde.
--
-- Fix   : statut IN ('paye','utilise') dans les 5 fonctions concernées.
--         Drop du trigger trg_reservations_terrain_scoring (appelle
--         ajouter_points_commande qui n'existe pas — l'architecture actuelle
--         compte directement depuis reservations_terrain, pas via prestataire_scores).
-- =============================================================================
...
BEGIN
  ...
  cmds_terrain AS (
    ...
    WHERE rt.statut IN ('paye', 'utilise')              -- FIX: inclut les réservations validées
...
-- ── 6. Nettoyage : trigger terrain inutile (appelle ajouter_points_commande
--       inexistant — les réservations terrain sont déjà comptées via SELECT
--       direct dans les CTEs cmds_terrain ci-dessus) ──────────────────────────
