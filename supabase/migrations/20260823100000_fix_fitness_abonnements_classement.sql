-- ===========================================================================
-- LASSI — Fix fitness : abonnements pris en compte dans classement
-- ---------------------------------------------------------------------------
-- Problème : les abonnements n'apparaissaient pas dans le classement sport
-- parce que le filtre date utilisait uniquement fac.date_achat.
-- Si le paiement OM avait un délai de traitement ou si le prestataire avait
-- des abonnements dont la date_achat était légèrement en dehors de la fenêtre,
-- ils n'étaient pas comptés.
--
-- Fix : dans cmds_fitness, on accepte l'abonnement si SOIT
--   (a) fac.date_achat est dans la période courante (comportement actuel)
--   (b) la payout_queue associée a été créée dans la période courante
--       (payment confirmé = argent reçu = abonné réel)
--
-- Couvre aussi les abonnements sans payment_intent_id (créés manuellement
-- ou via un flux sans paiement Wave/OM) en les excluant proprement.
-- ===========================================================================


-- ── 1. get_classement_live_sous_categorie (utilisé par "Ma catégorie – Cette semaine") ──
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  -- Abonnements achetés cette semaine
  ...
  -- Abonnements dont le paiement a été confirmé cette semaine (payout créé)
  cmds_fitness_by_payout AS (
    ...
    CROSS JOIN week_bounds wb
    WHERE pq.created_at >= wb.week_start AND pq.created_at < wb.week_end
...
-- ── 2. get_classement_live_mondial (utilisé par "National – Ce mois-ci") ─────
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  -- Abonnements dont le paiement a été confirmé ce mois (payout créé)
  cmds_fitness_by_payout AS (
    SELECT s.id AS shop_id, fac.client_id
    FROM fitness_abonnements_clients fac
    JOIN shops s ON s.merchant_id = fac.prestataire_id
    JOIN payout_queue pq ON pq.payment_intent_id = fac.payment_intent_id
    CROSS JOIN month_bounds mb
    WHERE pq.created_at >= mb.mois_start AND pq.created_at < mb.mois_end
      AND pq.statut IN ('queued', 'processing', 'paid')
      AND fac.payment_intent_id IS NOT NULL
      AND s.merchant_id IS NOT NULL
  ),
...
-- ── 3. calcul_classements_semaine (cron du dimanche soir) ────────────────────
...
BEGIN
  ...
  cmds_fitness_by_payout AS (
    ...
    JOIN shops s ON s.merchant_id = fac.prestataire_id
    JOIN payout_queue pq ON pq.payment_intent_id = fac.payment_intent_id
    WHERE pq.created_at >= v_week_start AND pq.created_at < v_week_end
      AND pq.statut IN ('queued', 'processing', 'paid')
      AND fac.payment_intent_id IS NOT NULL AND s.merchant_id IS NOT NULL
  ),
  cmds_terrain AS (
...
-- ── 4. calcul_classements_mois — Mondial + clients (cron) ────────────────────
...
BEGIN
  ...
  cmds_fitness_by_payout AS (
    ...
    JOIN shops s ON s.merchant_id = fac.prestataire_id
    JOIN payout_queue pq ON pq.payment_intent_id = fac.payment_intent_id
    WHERE pq.created_at >= v_mois_start AND pq.created_at < v_mois_end
      AND pq.statut IN ('queued', 'processing', 'paid')
      AND fac.payment_intent_id IS NOT NULL AND s.merchant_id IS NOT NULL
  ),
  cmds_terrain AS (
...
BEGIN
  ...
  RAISE NOTICE '[lassi] classements semaine recalculés (fitness payout fix) : % lignes', v_n;
  SELECT public.calcul_classements_mois() INTO v_n;
  RAISE NOTICE '[lassi] classements mois recalculés (fitness payout fix) : % lignes', v_n;
