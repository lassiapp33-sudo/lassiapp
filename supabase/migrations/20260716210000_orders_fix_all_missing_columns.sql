-- ===========================================================================
-- FIX COMPLET : toutes les colonnes manquantes dans orders + create_order_atomic
-- ---------------------------------------------------------------------------
-- Problème : la table orders a été créée sans certaines colonnes que les
-- migrations suivantes (ou create_order_atomic) supposent existantes.
-- Solution : ADD COLUMN IF NOT EXISTS pour chaque colonne — sans risque.
-- ===========================================================================

-- ① Colonnes manquantes sur orders (ordre exact du INSERT de create_order_atomic)
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS client_name     TEXT,
  ADD COLUMN IF NOT EXISTS discount_amount NUMERIC DEFAULT 0,
  ADD COLUMN IF NOT EXISTS promo_label     TEXT,
  ADD COLUMN IF NOT EXISTS note            TEXT,
  ADD COLUMN IF NOT EXISTS idempotency_key TEXT;

-- ② Autres colonnes utilisées par le code mais potentiellement absentes
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS voice_note_url  TEXT,
  ADD COLUMN IF NOT EXISTS prep_time       TEXT,
  ADD COLUMN IF NOT EXISTS refusal_reason  TEXT,
  ADD COLUMN IF NOT EXISTS refused_at      TIMESTAMPTZ;

-- ③ Index d'idempotence (scopé par client) — idempotent si existant
CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_idempotency_client
  ON public.orders (client_id, idempotency_key)
  WHERE idempotency_key IS NOT NULL;

-- ④ Re-créer create_order_atomic avec SET row_security = off
--    (fix critique : sans ça, l'INSERT dans order_items échoue sur la RLS)
CREATE OR REPLACE FUNCTION public.create_order_atomic(
  p_shop_id         UUID,
  p_client_id       UUID,
  p_client_name     TEXT,
  p_total           NUMERIC,
  p_discount_amount NUMERIC,
  p_promo_label     TEXT,
  p_order_type      TEXT,
  p_note            TEXT,
  p_idempotency_key TEXT,
  p_items           JSONB   -- [{ product_name, qty, unit_price }]
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
SET row_security = off
AS $$
DECLARE
  v_order_id UUID;
BEGIN
  INSERT INTO public.orders (
    shop_id, client_id, client_name,
    total, discount_amount, promo_label,
    status, pay_method, order_type,
    note, idempotency_key
  )
  VALUES (
    p_shop_id, p_client_id, p_client_name,
    p_total, p_discount_amount, p_promo_label,
    'new', 'wave', p_order_type,
    p_note, p_idempotency_key
  )
  RETURNING id INTO v_order_id;

  INSERT INTO public.order_items (order_id, product_name, qty, unit_price)
  SELECT
    v_order_id,
    (item ->> 'product_name')::TEXT,
    (item ->> 'qty')::INTEGER,
    (item ->> 'unit_price')::NUMERIC
  FROM jsonb_array_elements(p_items) AS item;

  IF p_total > 100000 THEN
    BEGIN
      PERFORM public.raise_fraud_flag(
        'high_amount', 'order', v_order_id::text, 'medium',
        jsonb_build_object('total', p_total, 'shop_id', p_shop_id, 'client_id', p_client_id)
      );
    EXCEPTION WHEN OTHERS THEN
      NULL; -- non bloquant si la fonction n'existe pas encore
    END;
  END IF;

  RETURN jsonb_build_object('id', v_order_id);
END;
$$;

REVOKE EXECUTE ON FUNCTION public.create_order_atomic(
  UUID, UUID, TEXT, NUMERIC, NUMERIC, TEXT, TEXT, TEXT, TEXT, JSONB
) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.create_order_atomic(
  UUID, UUID, TEXT, NUMERIC, NUMERIC, TEXT, TEXT, TEXT, TEXT, JSONB
) TO service_role;
