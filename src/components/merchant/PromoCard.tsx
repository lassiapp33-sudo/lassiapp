...
export function PromoCard({ promo, products, onEdit, onDelete, onToggle }: PromoCardProps) {
  ...
  const product =
    promo.cibleType === 'produit' && promo.cibleId
      ? products.find(p => p.id === promo.cibleId)
      : null;

  const cibleLabel =
    promo.cibleType === 'vitrine'
      ? 'Toute la vitrine'
      : promo.cibleType === 'categorie'
        ? `Catégorie : ${promo.cibleId ?? '—'}`
        : product
  ...
  const valeurLabel =
    promo.type === 'pourcentage'
      ...
      : promo.type === 'montant_fixe'
        ...
        : promo.type === 'quantite_offerte'
          ? `${promo.valeur}+1 offert`
          : `→ ${formatPrice(promo.valeur)}`;
  ...
  return (
    <View style={s.card}>
      ...
      <View style={s.bot}>
        ...
        {promo.dateFin && <Text style={s.date}>Jusqu'au {formatDateDMY(promo.dateFin)}</Text>}
        {promo.montantMin > 0 && <Text style={s.date}>Min. {formatPrice(promo.montantMin)}</Text>}
        <View style={{ flex: 1 }} />
