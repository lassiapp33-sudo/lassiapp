...
import { IcoWave, IcoNeighborhood, IcoCart, IcoChat } from '../common/LassiIcons';
...
const FEATURES: Feature[] = [
  { id: 'neighborhood', text: 'Découvre les commerces et prestataires de ton quartier' },
  { id: 'cart', text: 'Commande en quelques clics et suis ta commande en direct' },
  { id: 'chat', text: 'Discute directement avec les commerçants' },
  { id: 'star', text: 'Cumule des points et grimpe dans le classement "Top clients"' },
];

function FeatureIcon({ id }: { id: string }) {
  if (id === 'neighborhood') return <IcoNeighborhood size={22} />;
  if (id === 'cart') return <IcoCart size={22} />;
  if (id === 'chat') return <IcoChat size={22} />;
          ...
          <View style={s.list}>
            {FEATURES.map(f => (
              <View key={f.text} style={s.row}>
                <View style={s.rowIconBox}><FeatureIcon id={f.id} /></View>
