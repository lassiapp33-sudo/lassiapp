...
import {
  Animated, Image, Linking, Platform, ScrollView,
      ...
      <View style={styles.sheetCard}>
        ...
        <View style={styles.sheetInfo}>
          <View style={styles.sheetTopRow}>
            <Text style={styles.sheetName} numberOfLines={1}>{shop.name}</Text>
...
export default function MapScreen({ onBack, onShopPress, excludeShopId }: Props) {
  ...
  useEffect(() => {
    if (!selected) { setSelectedZone(''); return; }
    if (selected.zone) { setSelectedZone(selected.zone); return; }
  ...
  const distanceM = (selected && coords && selected.latitude && selected.longitude)
...
const styles = StyleSheet.create({
  ...
  topBar: {
    ...
    zIndex: 30, flexDirection: 'row', alignItems: 'center', gap: 10,
  },
  backBtn: {
  ...
  // Recentrer
  recenter: {
    position: 'absolute', right: 16, bottom: 250, zIndex: 30,
  ...
  // Indicateurs
  loadingBadge: {
    position: 'absolute', right: 16, top: TOP_INSET + 108, zIndex: 30,
  },
  emptyBadge: {
    position: 'absolute', bottom: 200, left: 40, right: 40, zIndex: 30,
  ...
  sheetName: {
    color: W, fontFamily: fonts.titleXL,
    fontSize: 16, flexShrink: 1,
  },
  vipBadge: {
    color: colors.accent, fontFamily: fonts.ui, fontSize: 9,
    paddingHorizontal: 6, paddingVertical: 2,
    backgroundColor: 'rgba(253,207,52,.15)', borderRadius: 6,
  },
  sheetMeta: {
    flexDirection: 'row', alignItems: 'center',
    gap: 9, marginTop: 4, flexWrap: 'wrap',
  },
