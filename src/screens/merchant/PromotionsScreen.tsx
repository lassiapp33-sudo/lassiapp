...
function typeValeurLabel(type: PromoType): string {
  switch (type) {
    case 'pourcentage':
      return 'Valeur (%) entre 1 et 100';
    case 'montant_fixe':
      return 'Montant en FCFA';
    case 'quantite_offerte':
      return 'X achetés (ex: 2 → 2+1 offert)';
    case 'prix_barre':
      return 'Nouveau prix en FCFA';
  }
}
...
export default function PromotionsScreen({ onBack }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setPromos(await promoService.getShopPromos(shopId));
    } catch {
      /* silencieux */
    } finally {
      setLoading(false);
  ...
  const handleToggle = async (promo: Promotion, val: boolean) => {
    ...
    try {
      await promoService.togglePromo(promo.id, val);
    } catch {
      setPromos(ps => ps.map(p => (p.id === promo.id ? { ...p, actif: !val } : p)));
    }
  };
        ...
        {loading ? (
          ...
          <View style={styles.empty}>
            ...
            <Text style={styles.emptySub}>
              Crée ta première promo pour attirer plus de clients !
            </Text>
              ...
              <View style={styles.optionRow}>
                {PROMO_TYPES.map(pt => (
                  ...
                  >
                    <Text
                      style={[
                        styles.optionPillTxt,
                        form.type === pt.value && styles.optionPillTxtOn,
                      ]}
                    >
                      {pt.label}
                    </Text>
                ...
                {CIBLE_TYPES.map(ct => (
                  ...
                  >
                    <Text
                      style={[
                        styles.optionPillTxt,
                        form.cibleType === ct.value && styles.optionPillTxtOn,
                      ]}
                    >
                      {ct.label}
                    </Text>
                    ...
                    {products.map(p => (
                      ...
                      >
                        ...
                        <Text
                          style={[
                            styles.prodPillTxt,
                            form.cibleId === p.id && styles.prodPillTxtOn,
                          ]}
                          numberOfLines={1}
                        >
                          {p.name}
                    ...
                    {storeCategories.map(cat => (
                      ...
                      >
                        ...
                        <Text
                          style={[
                            styles.prodPillTxt,
                            form.cibleId === cat.id && styles.prodPillTxtOn,
                          ]}
                        >
                          {cat.label}
                        </Text>
