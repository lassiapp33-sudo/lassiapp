...
const IcoWallet = () => (
  <Svg width={18} height={18} viewBox="0 0 24 24" fill="none" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round">
    <Path d="M20 12V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-2" stroke={colors.accent} />
...
export default function MerchantProfileScreen({ onBack, onStore, onVisibility, onRevenue, onPayments, onMyOrders, onLogout }: Props) {
  const t    = useT();
    ...
    <View style={styles.root}>
      ...
      >
        ...
        <View style={profileRowStyles.grp}>
          <ProfileOptionRow icon={<IcoGrid />}   title={t.profile.myStore}      subtitle={t.profile.myStoreSub}      onPress={onStore} />
          <ProfileOptionRow icon={<IcoTrend />}  title={t.profile.myVisibility} subtitle={t.profile.myVisibilitySub} onPress={onVisibility} />
          <ProfileOptionRow icon={<IcoDollar />} title={t.profile.myRevenue}    subtitle={t.profile.myRevenueSub}    onPress={onRevenue} />
          <ProfileOptionRow icon={<IcoWallet />} title={t.profile.myPayments}   subtitle={t.profile.myPaymentsSub}   onPress={onPayments} />
          <ProfileOptionRow icon={<IcoBag />}    title="Mes achats"             subtitle="Commandes passées chez d'autres" onPress={onMyOrders} last />
        </View>

        <Text style={profileRowStyles.secLbl}>{t.profile.preferences}</Text>
        <View style={profileRowStyles.grp}>
          <ProfileOptionRow
            icon={<IcoBell />} title={t.profile.notifications} subtitle={t.profile.notificationsSub}
          ...
          />
          <ProfileOptionRow icon={<IcoCard />} title={t.profile.merchantNumber} subtitle={t.profile.merchantNumberSub} last />
        </View>

        <Text style={profileRowStyles.secLbl}>{t.profile.helpAccount}</Text>
        <View style={profileRowStyles.grp}>
          <ProfileOptionRow icon={<IcoHelp />}    title={t.profile.helpSupport}  onPress={() => setShowHelp(true)} />
          <ProfileOptionRow icon={<IcoFlag />}    title="Signaler un problème"   onPress={() => setShowSignaler(true)} />
          <ProfileOptionRow icon={<IcoInfo />}    title="À propos"               onPress={() => setShowAPropos(true)} />
          <ProfileOptionRow
            icon={<IcoWhatsApp />}
            title={t.profile.whatsapp}
            subtitle={t.profile.whatsappSub}
            end="arrow"
            onPress={() => contacterServiceClient('Bonjour Lassi, je suis un prestataire et j\'ai besoin d\'aide avec mon compte.')}
          />
          <ProfileOptionRow icon={<IcoLogout />} title={t.profile.logout}        danger end="none" onPress={onLogout} />
          <ProfileOptionRow icon={<IcoTrash />}  title={t.profile.deleteAccount} danger end="none" onPress={() => setShowDeleteModal(true)} last />
        </View>
