...
export default function RevenueScreen({ onBack }: Props) {
  ...
  const now = new Date();
  const todayOrders =
    monthOffset === 0 ? orders.filter(o => isSameDay(new Date(o.createdAt), now)) : [];
    ...
    <View style={s.root}>
      ...
      {loading ? (
        ...
        >
          ...
          <View style={s.mainCard}>
            <View style={s.mainCardTop}>
              <View>
                <Text style={s.mainLabel}>
                  Total {isCurrentMonth ? 'ce mois' : monthLabel(monthOffset).split(' ')[0]}
                </Text>
                <Text style={s.mainAmount}>{monthTotal > 0 ? fcfaFull(monthTotal) : '0 FCFA'}</Text>
                <Text style={s.mainCount}>
                  {monthCount} commande{monthCount !== 1 ? 's' : ''}
                </Text>
              </View>
              <View style={s.mainRight}>
                ...
                {monthPend > 0 && (
                  <View style={[s.pill, s.pillPend]}>
                    <Text style={[s.pillTxt, { color: colors.muted }]}>
                      ⏳ {fcfaFull(monthPend)}
                    </Text>
                  </View>
                )}
            ...
            {monthTotal > 0 && (
              <Text style={s.progLbl}>
                {Math.round((monthConf / monthTotal) * 100)}% confirmé · {confCount}/{monthCount}{' '}
                commandes
              </Text>
            )}
          </View>
            ...
            <View style={s.todayRow}>
              <View style={s.todayCard}>
                ...
                <Text style={s.todayAmount}>
                  {todayOrders.length > 0 ? fcfaFull(todayTotal) : '—'}
                </Text>
              ...
              <View style={[s.todayCard, s.todayCardConf]}>
                ...
                <Text style={s.todayCount}>
                  {todayOrders.filter(o => o.status === 'done').length} cmd
                  {todayOrders.filter(o => o.status === 'done').length !== 1 ? 's' : ''}
                </Text>
              </View>
            </View>
          )}
          ...
          <View style={s.section}>
            ...
            <View style={s.chartBox}>
              ...
              <View style={s.chartLegend}>
                ...
                <View
                  style={[s.lgDot, { backgroundColor: 'rgba(253,207,52,.2)', marginLeft: 10 }]}
                />
          ...
          <View style={s.section}>
            <Text style={s.sectionTitle}>Détail — {monthLabel(monthOffset)}</Text>
            {daySats.length === 0 ? (
              <View style={s.empty}>
              ...
              <View style={s.debtCard}>
                <View>
                  ...
                  <Text style={s.debtSub}>
                    {debtors.filter(d => d.amount > 0).length} client
                    {debtors.filter(d => d.amount > 0).length !== 1 ? 's' : ''} avec solde impayé
                  </Text>
                </View>
