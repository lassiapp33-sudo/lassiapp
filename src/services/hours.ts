 ...
 * @param hours         Horaires hebdo (null ou objet vide = horaires non définis → ouvert par défaut)
 * @param manuallyClose true = override "exceptionnellement fermé"
 *
 * Les jours manquants sont comblés par DEFAULT_WEEK_HOURS, ce qui garantit
 * un résultat identique quel que soit l'écran qui appelle cette fonction.
 */
export function computeStatus(hours: WeekHours | null, manuallyClose: boolean): ShopStatus {
  if (manuallyClose) {
    return { isOpen: false, label: 'Exceptionnellement fermé', nextChange: '' };
  }

  // Null ou objet vide = horaires non configurés → ouvert par défaut
  if (!hours || Object.keys(hours).length === 0) {
    return { isOpen: true, label: 'Ouvert', nextChange: '' };
  }

  // Fusionner avec les defaults pour que les 7 jours soient toujours présents
  const effective = { ...DEFAULT_WEEK_HOURS, ...hours } as WeekHours;
