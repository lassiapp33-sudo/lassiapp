...
function rowToDebtor(row: Record<string, any>): Debtor {
  const daysSince = Math.floor((Date.now() - new Date(row.last_updated).getTime()) / 86400000);
...
export async function addDebtor(shopId: string, name: string, phone?: string): Promise<Debtor> {
  const { data, error } = await supabase
    .from('debts')
    .insert({
      shop_id: shopId,
...
export async function markPaid(debtId: string): Promise<void> {
  ...
  await supabase
    ...
    .insert({ debt_id: debtId, amount: 0, note: 'Paiement complet' });
}
