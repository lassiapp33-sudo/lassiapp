  ...
  if (existing) {
    const { data, error } = await supabase
      ...
      .update({
        ...
        opening_hours:     params.openingHours   !== undefined ? params.openingHours : existing.openingHours,
        is_manually_closed: params.isManuallyClose !== undefined ? params.isManuallyClose : existing.isManuallyClose,
      })
      .eq('id', existing.id)
      .select()
      .single();
...
export async function updateManuallyClose(
  shopId: string,
  closed: boolean,
): Promise<void> {
  const { error } = await supabase
