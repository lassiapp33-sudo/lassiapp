...
import { computeStatus, type WeekHours } from './hours';
...
function rowToResult(row: Record<string, any>): ShopResult {
  ...
  return {
    ...
    isOpen: computeStatus(row.opening_hours as WeekHours | null, Boolean(row.is_manually_closed))
      .isOpen,
    isVip,
  ...
  if (position) {
    results = results.map(s =>
      s.latitude != null && s.longitude != null
        ? {
            ...s,
            distance: calcDistanceMeters(position.lat, position.lng, s.latitude, s.longitude),
          }
        : s,
    );
  }
...
export async function loggerQuestionSansReponse(texte: string): Promise<void> {
  try {
    await supabase.from('faq_misses').insert({ question: texte.slice(0, 500) });
  } catch {
